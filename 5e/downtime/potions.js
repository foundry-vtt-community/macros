...
async function main(days){
    ...
    let hasprof = character.items.filter( item => item.name.match(/^Alchemist.s Supplies$/) && Object.prototype.hasOwnProperty.call(item.system, "proficient"));
    let rollmethod = ``
    ...
    for (let iter = 0; iter < days; iter++) {
        let arcanerolleval = await new Roll(rollmethod).evaluate({async: true});
        // Reset the work and multiplier per iter
        let multiplier = basemultiplier;
        let work = 0;
        // Find the rolled number, maybe a better way of doing this
        const regex = /^\d+/;
        let temp = arcanerolleval.result.match(regex);
        let natural = Number(temp[0]);
