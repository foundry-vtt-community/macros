/*
* Gets a list of selected tokens (or defaults to the user's character), provides a choice of
* save, skill, or ability check, and then makes a roll for all the selected tokens with that check. It spits out
* the poorly-formatted results to chat (or whispers to GM if you change line 8 to true).
                    ...
                    for (let a of targetActors) {
                        ...
                        switch (checkType) {
                            case "save":
                                ...
                                messageContent += `${name}: <b>[[1d20+${mod}]]</b> (${game.dnd5e.config.abilities[id]})<br>`;
                                break;
                            case "ability":
                                mod = a.data.data.abilities[id].mod + a.data.data.abilities[id].checkBonus;
                                messageContent += `${name}: <b>[[1d20+${ability.mod}]]</b> (${game.dnd5e.config.abilities[id]})<br>`;
                                break;
                            case "skill":
                                mod = a.data.data.skills[id].total;
                                messageContent += `${name}: <b>[[1d20+${mod}]]</b> (${game.dnd5e.config.skills[id]} [${a.data.data.skills[id].ability}])<br>`;
                                break;
                            default:
